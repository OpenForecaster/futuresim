#!/usr/bin/env python3
"""
Evaluate a model on synthetic QA pairs generated by generate_qa.py.

Reads JSONL question files from a syntheticQA output folder, sends each
question + background to the evaluation model, and saves per-source outputs.

The eval model must provide its final answer inside <answer>...</answer> tags.
...
(e.g., /lustre/scratch/nchandak/.../Qwen3-8B-1e-5-4096-4096/ which contains
global_step_120/, global_step_180/, etc.).

Usage:
    python syntheticQA/eval_qa.py --questions /path/to/syntheticqa/run_folder/
    python syntheticQA/eval_qa.py --model /fast/nchandak/models/Qwen3-8B --questions /path/to/folder/
    python syntheticQA/eval_qa.py --model /fast/nchandak/models/gpt-oss-20b --questions /path/to/folder/ --effort medium
    python syntheticQA/eval_qa.py --checkpoints /path/to/training_run/ --questions /path/to/folder/
"""
...
# ---------------------------------------------------------------------------
# Harmony helpers (shared with generate_qa.py)
# ---------------------------------------------------------------------------
...
def discover_checkpoints(checkpoints_dir: str) -> List[tuple]:
    ...
    Expects a layout like:
        Qwen3-8B-1e-5-4096-4096/
            ...
            global_step_180/
                Qwen3-8B-1e-5-4096-4096-synthetic-sft-step180/
                huggingface/

    The model subfolder shares the same prefix as the parent training run
    folder name (e.g. both start with "Qwen3-8B-1e-5-4096-4096").
    ...
    run_name = root.name  # e.g. "Qwen3-8B-1e-5-4096-4096"
    ...
    for d in sorted(root.iterdir()):
        ...
        # Find the model subfolder whose name starts with the run name
        # (e.g. "Qwen3-8B-1e-5-4096-4096-synthetic-sft-step120")
        model_path = None
        ...
        # Fallback: any subfolder (not "huggingface") with .safetensors files
...
def build_eval_prompt(question: str, background: str, no_think: bool = False) -> str:
    ...
    parts.append(
        ...
        f"\nFor example: <answer>The answer is X.</answer>{suffix}"
    )
...
def _run_single_model(
    model_path: str,
    output_dir: Path,
    source_qa: Dict[str, List[dict]],
    questions_folder_name: str,
    args,
    use_harmony: bool,
):
    """Run evaluation for a single model and save results to output_dir."""
    # if output_dir.exists() and any(output_dir.iterdir()):
    #     print(f"Skipping eval: output directory already exists with files: {output_dir}")
    #     return
    ...
    for source_name, records in source_qa.items():
        for rec in records:
            ...
            prompt_text = build_eval_prompt(question, background, no_think=args.no_think)
...
def run_evaluation(args):
    ...
    if args.checkpoints:
        # Checkpoint mode: discover all checkpoints and determine model type
        ...
        # Use first checkpoint to determine model type
    ...
    if checkpoints:
        # --- Checkpoint mode: iterate over all checkpoints ---
        # Use the training run folder name as the base eval directory name
        ...
        if use_harmony:
            base_eval_dir_name = f"{run_name}_{args.effort}_{timestamp}"
        else:
            base_eval_dir_name = f"{run_name}{think_suffix}_{timestamp}"

        base_output_dir = Path(args.output_base) / questions_folder_name / base_eval_dir_name
        ...
        think_suffix = "_no_think" if (not use_harmony and args.no_think) else "_thinking"
        eval_dir_name = f"{eval_model_short}{think_suffix}_{timestamp}"
        if use_harmony:
            eval_dir_name = f"{eval_model_short}_{args.effort}_{timestamp}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a model on synthetic QA pairs using vLLM batch generation."
    )
    parser.add_argument(
    ...
    if args.checkpoints:
        # Peek at first checkpoint to determine model type for defaults
