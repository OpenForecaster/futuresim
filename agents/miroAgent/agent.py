...
class MiroBasicAgent(BasicAgent):
    ...
    def act(self, doc_interface, forecast_interface, current_date: date) -> List[Dict[str, Any]]:
        ...
        messages = [{"role": "user", "content": self._build_miro_instructions(current_date)}]
        all_forecasts, context_limit_hit = self._run_miro_action_loop(
            messages=messages,
    ...
    def _build_miro_instructions(self, current_date: date) -> str:
        base_prompt = self._build_instructions(current_date)
        return self._rewrite_basic_prompt_for_miro(base_prompt)

    def _rewrite_basic_prompt_for_miro(self, base_prompt: str) -> str:
        prefix = base_prompt
    ...
    def _run_miro_action_loop(
        ...
        *,
        ...
        self._inject_tool_schema(messages, tools)
        ...
        while not budget.is_exhausted():
            ...
            if parsed.action_type == "submit":
                before_len = len(messages)
                submitted = self._miro_handle_submit(
                    messages=messages,
                    forecast_interface=forecast_interface,
                    parsed=parsed,
                    tool_call=tool_calls[0] if tool_calls else None,
    ...
    @staticmethod
    def _inject_tool_schema(messages: List[Dict[str, Any]], tools: List[Dict[str, Any]]) -> None:
        if not messages:
            return
        first = messages[0]
    ...
    def _apply_tool_result_retention(
        self,
        messages: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        raw_keep_last = getattr(self.config, "tool_result_keep_last", -1)
        keep_last = -1 if raw_keep_last is None else int(raw_keep_last)
        if keep_last < 0:
            return messages
        ...
        replayable_indices = [idx for idx in user_or_tool_indices if idx != first_user_idx]
        if keep_last > 0:
            keep_indices.update(replayable_indices[-keep_last:])

        for idx, message in enumerate(messages_copy):
            if message.get("role") not in ("user", "tool") or idx in keep_indices:
                continue
            message["content"] = self._OMITTED_TOOL_RESULT_TEXT

        return messages_copy

    def _miro_handle_query(
        self,
        *,
        messages: List[Dict[str, Any]],
        parsed: ParsedAction,
...
class MiroAllQAgent(AllQAgent, MiroBasicAgent):
    ...
    def _rewrite_allq_warmup_prompt_for_miro(self, base_prompt: str) -> str:
        prefix = base_prompt
        if "## RESPONSE FORMAT" in base_prompt:
            prefix = base_prompt.split("## RESPONSE FORMAT", 1)[0].rstrip()
