...
# OPSD for Forecasting — v3 matching verl configs where applicable.
# Student/Teacher: Qwen3-4B-Instruct-2507
#
# Uses forecast-specific OPSD entrypoint (main_opsd_forecast.py) with teacher prompt
# that matches <answer>/<probability> tags (prevents format drift).
#
# bash examples/train/forecast/run_opsd_forecast_qwen3_4b.sh
...
RUN_NAME="opsd_Qwen3-4B_v3_forecastTeacher_lr${LR}"
...
uv run --isolated --extra fsdp -m examples.train.forecast.main_opsd_forecast \
