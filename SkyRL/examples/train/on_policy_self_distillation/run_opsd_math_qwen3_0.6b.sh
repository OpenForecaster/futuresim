...
MODEL="/fast/nchandak/models/Qwen3-0.6B"
...
uv run --isolated --extra fsdp -m examples.train.on_policy_self_distillation.main_opsd \
  ...
  trainer.run_name="opsd_aime_qwen3_0.6b_self_distill_bs56" \
  trainer.resume_mode=latest \
  trainer.export_path="$HOME/exports/aime_opsd_0.6b_self_distill" \
  trainer.hf_save_interval=10 \
  trainer.max_ckpts_to_keep=3 \
  trainer.ckpt_interval=10 \
  trainer.ckpt_path="$HOME/ckpts/aime_opsd_0.6b_self_distill" \
  $@
