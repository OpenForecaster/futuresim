...
def _build_overrides(config: Dict[str, Any], train_path: str, val_path: str) -> List[str]:
    ...
    if mode == "eval":
        ...
        min_train_batch_size = max(1, math.ceil(gpus / max(1, n_samples_per_prompt)))
        if train_rows < min_train_batch_size:
            raise ValueError(
                "Eval mode: train row count must be at least "
                f"{min_train_batch_size} for {gpus} GPUs and n_samples_per_prompt={n_samples_per_prompt} "
                f"(so train_batch_size * n_samples_per_prompt >= {gpus}); got {train_rows} in {train_path}. "
