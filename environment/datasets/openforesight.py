...
class OpenForesightFetcher(DataFetcher):
    ...
    def _load_split_dataframe(self, path: str, split: str, cache_dir: Optional[str]) -> pd.DataFrame:
        ...
        if os.path.isdir(path):
            parquet_root = path
            parquet_files = sorted(glob.glob(os.path.join(parquet_root, f"{split}-*.parquet")))
            if not parquet_files and os.path.isdir(os.path.join(path, "data")):
                parquet_root = os.path.join(path, "data")
                parquet_files = sorted(glob.glob(os.path.join(parquet_root, f"{split}-*.parquet")))
    ...
    def load_from_cache(
        ...
        path = self.dataset_path or "nikhilchandak/OpenForesight"
